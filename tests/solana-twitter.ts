...
describe("solana-twitter", () => {
  ...
  it("can send a new tweet", async () => {
    ...
    // console.log("publicKey", tweet.publicKey);
    // console.log("author", program.provider.wallet.publicKey);
    // console.log("systemProgram", anchor.web3.SystemProgram.programId);

    await program.rpc.sendTweet("xord", "Hummus, am I right?", {
      accounts: {
        // Accounts here...
  ...
  it("can fetch all tweets", async () => {
    ...
    console.log("tweetAccounts", tweetAccounts.length);
  ...
  it("can filter tweets by author", async () => {
    ...
    console.log("hasTweets", tweetAccounts.length);
    ...
    const tweetAccounts = await program.account.tweet.all([
      {
        memcmp: {
          ...
          bytes: bs58.encode(Buffer.from("xord")),
        },
      },
    ]);

    console.log("hello", tweetAccounts.length);

    // assert.equal(tweetAccounts.length, 1);
    assert.ok(
      tweetAccounts.every((tweetAccount) => {
        return tweetAccount.account.topic === "xord";
      })
    );
  });
});
